...
void cb_points_up(const sensor_msgs::PointCloud2::ConstPtr& msg){
  ...
  if (diff > ros::Duration(0.2)){
    ...
    m_octree->insertPointCloud(temp_pcl, octomap::point3d(map_t_up(0,3),map_t_up(1,3),map_t_up(2,3)),18.0);
...
void cb_points_front(const sensor_msgs::PointCloud2::ConstPtr& msg){
  ...
  if (diff > ros::Duration(0.2)){
    ...
    pcl::transformPointCloud(*cloudIn, *tran_pc, map_t_front);
    ...
    m_octree->insertPointCloud(temp_pcl, octomap::point3d(map_t_front(0,3),map_t_front(1,3),map_t_front(2,3)),18.0);
    // *ptr_pc_accumulated += *final_pc;
    // if (local_map){
